...
class PostDB:
    ...
    def init_db(self):
        # Create Posts table if it doesn't exist, now including image_url
        self.conn.execute('''
            CREATE TABLE IF NOT EXISTS Posts (
                ...
                image_url TEXT NOT NULL  -- Added image URL field
            )
        ''')
        self.conn.commit()

    def add_post(self, title, pages, price, amount, genre, author, series, image_url):
        # Include image_url in the insert statement
        self.conn.execute('''
            INSERT INTO Posts (title, pages, price, amount, genre, author, series, image_url) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (title, pages, price, amount, genre, author, series, image_url))
        self.conn.commit()
